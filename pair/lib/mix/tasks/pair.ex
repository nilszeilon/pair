defmodule Mix.Tasks.Pair do
  ...
  Sessions on the pair socket are auto-discovered and served via ttyd.
  Create them with `tmux -L pair new -s name agent`.
  ...
  def run(args) do
    case args do
      ["server" | _] ->
        ...
        IO.puts("Create sessions: tmux -L pair new -s name agent")
